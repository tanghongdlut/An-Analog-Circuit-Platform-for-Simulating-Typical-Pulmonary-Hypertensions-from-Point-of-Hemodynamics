...
% cham==2, The blood pressure of right ventricle
% cham==3, The blood pressure of left atrium
% cham==4, The blood pressure of right atrium

function P=mycallP(Vx,t,Fcon,cham,BN1,BN2,mPAP)

%The values of parameters in P-V relationship of four chambers
%----LV         RV         LA        RA   
Ees=[4.3       0.8         0.3       0.3    ];
P0=[1.7        0.67        0.5       0.5    ];
V0=[25         25          20        20     ];
...
if cham==1  % left ventricle
    ...
    Ped=P0(cham)*abs(exp(    lamda(cham)  *   (   Vx  -  V0(cham)   )      )-1);
    P=En.*Pes+(1-En).*Ped;
    ...
    k1=0.0013; k2=0.0008;
   if mPAP<50
       Pes=a.*(Ees(cham)+(k1*BN1)).*(Vx-Vd(cham));
   else
       Pes=a.*(Ees(cham)+(k1*BN1)+k2*BN2).*(Vx-Vd(cham));
   end
    Ped=P0(cham)*abs(exp(    lamda(cham) *   (   Vx  -  V0(cham)   )      )-1);
    P=En.*Pes+(1-En).*Ped;   

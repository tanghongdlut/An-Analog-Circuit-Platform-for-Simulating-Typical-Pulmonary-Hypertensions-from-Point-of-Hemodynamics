...
% Then, plot the blood pressure and flow in specified time interval
% 
...
%beatNum=2
for num=1:samNum    
    Plpap(num,:)=allP(samNum+num,20);
    Plpad(num,:)=allP(samNum+num,21);
    Plpv(num,:)=allP(samNum+num,24);
    Prv(num,:)=allP(samNum+num,18);
    num=num+1;
end

for num1=samNum+1:2*samNum
    Plpap(num1,:)=allP(200*samNum+num1,20);
    Plpad(num1,:)=allP(200*samNum+num1,21);
...
for num2=2*samNum+1:3*samNum
    ...
    Plpad(num2,:)=allP(500*samNum+num2,21);
...
for num3=3*samNum+1:4*samNum
    ...
    Plpad(num3,:)=allP(700*samNum+num3,21);
...
for num4=4*samNum+1:5*samNum
    ...
    Plpad(num4,:)=allP(887*samNum+num4,21);
...
%  legend('\itP\rm_l_p_a_p','\itP\rm_l_p_a_d','\itP\rm_l_p_v','\itP\rm_r_v')
